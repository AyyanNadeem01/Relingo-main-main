...
export default function QuizPlayer({ quizId, lessonId: _lessonId, onComplete }: QuizPlayerProps) {
  ...
  const { data: questions = [], isLoading } = useQuery<Question[]>({
    ...
    queryFn: async () => {
      ...
      // Ensure we're using the correct language with multiple fallbacks
      const currentLanguage = i18n.language || localStorage.getItem('i18nextLng') || 'no';
      console.log('🔍 Final language to use:', currentLanguage);
      
      // Double-check language consistency
      if (i18n.language !== currentLanguage) {
        console.warn('⚠️ Language mismatch detected! i18n.language:', i18n.language, 'vs currentLanguage:', currentLanguage);
        // Force i18n to use the correct language
        await i18n.changeLanguage(currentLanguage);
      }
      
      const data = await getQuestions(quizId, currentLanguage);
      console.log('✅ Loaded questions:', data.map((q, index) => ({
        questionNumber: index + 1,
        id: q.id,
        type: q.type,
        prompt: q.prompt.substring(0, 50) + '...',
        language: currentLanguage,
        hasChoices: !!q.meta?.choices,
        choicesCount: q.meta?.choices?.length || 0
      })));
      return data;
    },
    // Ensure the query refetches when language changes
    staleTime: 0,
    refetchOnWindowFocus: false,
    // Force refetch if language changes
    refetchOnMount: true,
  });
  ...
  useEffect(() => {
    console.log('🔄 Language changed, resetting quiz state');
    ...
    // Force a complete refetch of questions with new language
    queryClient.invalidateQueries({ 
      queryKey: ['questions', quizId, i18n.language],
      exact: true 
    });
    
    // Also invalidate any cached questions to ensure fresh data
    queryClient.invalidateQueries({ 
      queryKey: ['questions', quizId],
      exact: false 
    });
  }, [i18n.language, quizId, queryClient]);
  ...
  console.log('Quiz state:', { 
    ...
    currentQuestionPrompt: currentQuestion?.prompt?.substring(0, 50),
