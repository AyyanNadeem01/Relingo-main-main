...
export async function getQuestions(quizId: string, language: string = 'no'): Promise<Question[]> {
  ...
  console.log('🔍 Current i18n language from localStorage:', localStorage.getItem('i18nextLng'));
  console.log('🔍 Quiz ID:', quizId);
  
  // Validate language parameter
  const validLanguages = ['en', 'no', 'tr'];
  const normalizedLanguage = validLanguages.includes(language) ? language : 'no';
  
  if (language !== normalizedLanguage) {
    console.warn(`⚠️ Invalid language '${language}', falling back to '${normalizedLanguage}'`);
  }
  ...
  return (data || []).map((question, index) => {
    ...
    // Special debugging for questions 3+ (index 2+)
    const isQuestion3Plus = index >= 2;
    if (isQuestion3Plus) {
      console.log(`🚨 QUESTION ${index + 1} DEBUG (ID: ${question.id}):`, {
        ...
        englishLength: question.prompt_en?.length || 0,
        turkishLength: question.prompt_tr?.length || 0,
        metaKeys: Object.keys(question.meta || {}),
        metaEnKeys: Object.keys(question.meta_en || {}),
        metaTrKeys: Object.keys(question.meta_tr || {}),
      });
    }
    ...
    if (normalizedLanguage === 'en' && question.prompt_en) {
      console.log(`  ✅ Using ENGLISH translation for question ${index + 1}`);
      prompt = question.prompt_en;
      meta = question.meta_en || question.meta;
    } else if (normalizedLanguage === 'tr' && question.prompt_tr) {
      console.log(`  ✅ Using TURKISH translation for question ${index + 1}`);
      prompt = question.prompt_tr;
      meta = question.meta_tr || question.meta;
    } else {
      console.log(`  ⚠️ Using NORWEGIAN fallback for question ${index + 1} (${normalizedLanguage} translation not available)`);
      console.log(`  🔍 Debug: prompt_en exists: ${!!question.prompt_en}, prompt_tr exists: ${!!question.prompt_tr}`);
      
      // AGGRESSIVE FIX: If we're requesting English or Turkish but it's not available, 
      // we should still try to use what's available, even if it's empty
      if (normalizedLanguage === 'en') {
        if (question.prompt_en) {
          prompt = question.prompt_en;
          meta = question.meta_en || question.meta;
          console.log(`  🔄 Fallback: Using available English translation`);
        } else {
          console.log(`  🚨 CRITICAL: Question ${index + 1} has NO English translation! Using Norwegian as fallback.`);
          // Keep Norwegian as fallback
        }
      } else if (normalizedLanguage === 'tr') {
        if (question.prompt_tr) {
          prompt = question.prompt_tr;
          meta = question.meta_tr || question.meta;
          console.log(`  🔄 Fallback: Using available Turkish translation`);
        } else {
          console.log(`  🚨 CRITICAL: Question ${index + 1} has NO Turkish translation! Using Norwegian as fallback.`);
          // Keep Norwegian as fallback
        }
      }
    }
    
    console.log(`  📤 Final question ${index + 1} prompt:`, prompt?.substring(0, 50));
